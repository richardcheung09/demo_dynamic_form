...
//       if (value && (!digits || ![10, 11].includes(digits.length))) {
//         toast.error(
...
// // export default function EditForm<T extends Entity>(props: EditFormProps<T>) {
...
// //   const [listFieldSize, setListFieldSize] = useState(0);
// //   useEffect(() => {
...
// //           for (const target of (event.target as HTMLFormElement).elements) {
// //             if (!(target as HTMLElement).hasAttribute('name')) continue;
// //             const targetName = (target as HTMLElement).getAttribute('name');
// //             if (targetName && updateTargets.has(targetName)) {
// //               // if target is radio button, only update if it is checked
// //               if ((target as HTMLInputElement).type === "radio" && !(target as HTMLInputElement).checked) continue;
// //               entity[targetName] = target.value;
// //             } else if (
// //                 targetName?.includes("_") &&
// //               updateTargets.has(targetName?.split("_")[0])
// //             ) {
// //               const entity_field = targetName?.split("_")[0];
// //               const entity_sub_field =targetName?
// //                 .split("_")
// //                 .slice(1)
// //                 .join("_");
